...
public class Starter {
  ...
  private static void bipeds() {
    ...
    //MultiLayerPerceptron.ActivationFunction activationFunction, int nOfInput, int[] innerNeurons, int nOfOutput, double eta, boolean toNormalize
    MultiLayerPerceptron mlp = new HebbianPerceptron(
        MultiLayerPerceptron.ActivationFunction.TANH,
    ...
    System.out.println(Arrays.toString(mlp.getNeurons()));
    System.out.println(mlp.getWeights()[0].length);
    System.out.println(mlp.getWeights()[0][0].length);
    double[] ws = mlp.getParams();
    IntStream.range(0, ws.length).forEach(i -> ws[i] = random.nextDouble() * 2d - 1d);
    mlp.setParams(ws);
    ...
    try {
      GridFileWriter.save(
          locomotion,
          Grid.create(1, 1, Pair.of("centralized", centralized)),
          300, 300, 1, 24,
          VideoUtils.EncoderFacility.JCODEC,
          new File("./biped-spectra.mp4"),
