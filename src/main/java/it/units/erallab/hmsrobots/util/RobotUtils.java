...
public class RobotUtils {
    ...
    private final static Map<String, BiFunction<Double, Double, Sensor>> PREDEFINED_SENSORS = new TreeMap<>(Map.ofEntries(
            ...
            Map.entry("l3", (x, y) -> new Normalization(new Lidar(10d, Map.of(lidarSide(x, y), 3)))),
    ...
    public static Function<Grid<Boolean>, Grid<? extends SensingVoxel>> buildSensorizingFunction(String name) {
        ...
        String high_worm = "high_worm-(?<noiseSigma>\\d+(\\.\\d+)?)-(?<cpg>[tf])";
        String medium_worm = "medium_worm-(?<noiseSigma>\\d+(\\.\\d+)?)-(?<cpg>[tf])";
        String low_worm = "low_worm-(?<noiseSigma>\\d+(\\.\\d+)?)-(?<cpg>[tf])";

        String high_biped = "high_biped-(?<noiseSigma>\\d+(\\.\\d+)?)-(?<cpg>[tf])";
        String medium_biped = "medium_biped-(?<noiseSigma>\\d+(\\.\\d+)?)-(?<cpg>[tf])";
        String low_biped = "low_biped-(?<noiseSigma>\\d+(\\.\\d+)?)-(?<cpg>[tf])";

        String empty = "empty";
        Map<String, String> params;
        if ((params = params(high_worm, name)) != null) {
                    ...
                    (x, y) -> {
                        ...
                        return new SensingVoxel(
                                Utils.ofNonNull(
                                        ...
                                        sensor("l3", x, y, body, x == body.getW() - 1),
        ...
        if ((params = params(medium_worm, name)) != null) {
            final Map<String, String> pars = params;
            double noiseSigma = Double.parseDouble(params.get("noiseSigma"));
            return body -> Grid.create(body.getW(), body.getH(),
                    (x, y) -> {
        ...
        if ((params = params(low_worm, name)) != null) {
            final Map<String, String> pars = params;
            double noiseSigma = Double.parseDouble(params.get("noiseSigma"));
            return body -> Grid.create(body.getW(), body.getH(),
                    (x, y) -> {
        ...
        if ((params = params(high_biped, name)) != null) {
            final Map<String, String> pars = params;
            double noiseSigma = Double.parseDouble(params.get("noiseSigma"));
            return body -> Grid.create(body.getW(), body.getH(),
                    (x, y) -> {
        ...
        if ((params = params(medium_biped, name)) != null) {
            final Map<String, String> pars = params;
            double noiseSigma = Double.parseDouble(params.get("noiseSigma"));
            return body -> Grid.create(body.getW(), body.getH(),
                    (x, y) -> {
        ...
        if ((params = params(low_biped, name)) != null) {
            final Map<String, String> pars = params;
            double noiseSigma = Double.parseDouble(params.get("noiseSigma"));
            return body -> Grid.create(body.getW(), body.getH(),
                    (x, y) -> {
                        ...
                        return new SensingVoxel(
                                Utils.ofNonNull(
                                        sensor("a", x, y, body, y == body.getH() - 1 || y == body.getH() - 2),
                                        //sensor("m", x, y, body, pars.get("malfunction").equals("t")),
                                        //sensor("t", x, y, body, y == 0),
                                        //sensor("vxy", x, y, body, y == body.getH() - 1),
                                        sensor("cpg", x, y, body, x == body.getW() - 1 && y == body.getH() - 1 && pars.get("cpg").equals("t"))
