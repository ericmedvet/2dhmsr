 ...
 * @author federico
 */
public class DistributedSensingWithAffordances extends AbstractController implements Snapshottable {
  ...
  private final List<Double> tempVotes;
  private boolean leftFirstContact;
  private boolean rightFirstContact;

  @JsonCreator
  public DistributedSensingWithAffordances(
    ...
    votes = new ArrayList<>();
    tempVotes = new ArrayList<>();
    rightFirstContact = false;
    leftFirstContact = false;
    reset();
  }

  public DistributedSensingWithAffordances(Grid<Voxel> voxels, int signals) {
    this(
        signals,
  ...
  public static int nOfOutputs(Voxel voxel, int signals) {
    return 2 + signals * Dir.values().length;
  }
  ...
  public void vote() {
    ...
    int numPos = (int) tempVotes.stream().filter(x -> x >= 0.0).count();
    int numNeg = (int) tempVotes.stream().filter(x -> x < 0.0).count();
