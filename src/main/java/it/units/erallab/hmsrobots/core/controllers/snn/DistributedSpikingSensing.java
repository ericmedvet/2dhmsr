...
public class DistributedSpikingSensing extends AbstractController {
  ...
  public DistributedSpikingSensing(Grid<? extends Voxel> voxels, int signals, SpikingFunction spikingFunction, ValueToSpikeTrainConverter valueToSpikeTrainConverter, SpikeTrainToValueConverter spikeTrainToValueConverter) {
    this(
        signals,
  ...
  public Grid<Double> computeControlSignals(double t, Grid<Voxel> voxels) {
    ...
    for (Grid.Entry<Voxel> entry : voxels) {
      ...
      //get inputs
      SortedSet<Double>[] lastSignals = getLastSignals(entry.key().x(), entry.key().y());
      SortedSet<Double>[] sensorValues = convertSensorReadings(entry.value().getSensorReadings(), inputConverters.get(entry.key().x(), entry.key().y()), t);
      SortedSet<Double>[] inputs = ArrayUtils.addAll(lastSignals, sensorValues);
      //compute outputs
      MultivariateSpikingFunction function = functions.get(entry.key().x(), entry.key().y());
