...
public class Outcome {
    ...
    public List<Gait> getMainGaits(double interval, double longestInterval) {
        List<Gait> gaits = computeGaits(
                getQuantizedFootprints(interval),
                2,
                (int) Math.round(longestInterval / interval),
    ...
    public double[] getSpectrumDescriptor() {
        double[] xSpectrum = getCenterPowerSpectrum(Outcome.Component.X, 0d, 5d, 2).stream()
                .mapToDouble(Outcome.Mode::getStrength)
                .toArray();
        double[] ySpectrum = getCenterPowerSpectrum(Outcome.Component.Y, 0d, 5d, 2).stream()
                .mapToDouble(Outcome.Mode::getStrength)
                .toArray();
        double[] spectrum = new double[4];

        System.arraycopy(xSpectrum, 0, spectrum, 0, 2);
        System.arraycopy(ySpectrum, 0, spectrum, 2, 2);
        return spectrum;
    }
